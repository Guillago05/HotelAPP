...
export const getHotelesDisponibles = async (ciudad, fecha_llegada, fecha_salida, personas) => {
    ...
    try {
        const response = await axios.get(`http://localhost:8080/searchresults?ciudad=${ciudad}&fecha_llegada=${fecha_llegada}&fecha_salida=${fecha_salida}&personas=${personas}`)
...
export const getPrecioMasBarato = async (nombreHotel, fecha_llegada, fecha_salida, personas) => {
    ...
    try {
        ...
        const response = await axios.get(`http://localhost:8080/habitacionesDisponibles?nombreHotel=${nombreHotelEncoded}&fecha_llegada=${fecha_llegada}&fecha_salida=${fecha_salida}&personas=${personas}`);
        const habitaciones = await response.data;
        if (!habitaciones.length) return null;//No hay habitaciones disponibles
